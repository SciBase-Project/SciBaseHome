    ...
    /*generateAllArticleIntScoreCsv: function() {
        ...
        var res = request({
            ...
            if (!err && response.statusCode === 200) {
                ...
                //console.log(JSON.stringify(response_body));
                try {
                    fs.writeFileSync(path.join(__dirname, "../public/files/graphcsv/", "generateAllArticleIntScore.csv"), module.exports.jsonToCsv(body), 'utf-8');
                    //fs.writeFileSync("/files/papers/Journal.csv",module.exports.jsonToCsv(body), 'utf-8');
                } catch (e) {
                    console.log("Error:", e.message);
                }
...
console.log("fetching : %s",i)
    ...
    search_article: function(search_object, callback) {
        ...
        // flag =1 for Production, 0 for test
        // to be set manually while deploying
        var flag = 0;

        var NEO4J_API_URL = "http://localhost:7474/db/data/transaction/commit";
        var NEO4J_USER = "neo4j";
        var NEO4J_PASS = "scibase";
        var article_query = "match (j:Journal)-[:" + module.exports.tVsPConvert(flag, "contains") + "]->(a:Article)-[:" + module.exports.tVsPConvert(flag, "authored_by") + "]->(b:Author)";
        article_query += (flag) ? "-[:PART_OF]->(c:Country)" : "";
        var author_query = "";
        var year_query = "";
        var ctrArticle = 0;
        var ctrYear = 0;
